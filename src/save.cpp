...
{
    if (xNorm < 0.35f) return "left";
    if (xNorm > 0.65f) return "right";
    return "center";
}
...
bool Impl::LoadGame(int slot)
{
    ...
    bgPath.clear();
    for (auto &[id, entry] : characters) SDL_DestroyTexture(entry.tex);
    characters.clear();
    charPaths.clear();
        ...
        if (key == "pc") {
            ...
            if (!val.empty()) {
                ...
                while (std::getline(ss, tok, ','))
                    if (!tok.empty()) callStack.push_back((size_t)std::stoull(tok));
            }
        } else if (key.size() > 4 && key.substr(0, 4) == "var.") {
            variables[key.substr(4)] = val;
        } else if (key == "bg") {
            if (!val.empty()) ShowBackground(val);
...
void Impl::DrawSaveLoadOverlay(bool isSaving)
{
    ...
    // Title
    SDL_Texture *titleTex = RenderText(isSaving ? "SAVE GAME" : "LOAD GAME",
                                       {180, 200, 255, 255});
    if (titleTex) {
        float tw, th;
    ...
    for (int i = 1; i <= 10; ++i) {
        ...
        SDL_Texture *slotTex = RenderText(label, ts.empty() ? SDL_Color{100, 100, 100, 255}
