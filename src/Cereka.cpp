...
public:
  ...
  bool InitGame(const char *title, int width, int height, bool fullscreen) {
    ...
    this->font =
        text_renderer::OpenFont("assets/fonts/Montserrat-Medium.ttf", 36);

    this->textBox = CreateSolidTexture(
        screenWidth, static_cast<int>(screenHeight * 0.25f), 255, 255, 0, 255);

    this->nameBox = CreateSolidTexture(
        300, 60, 0, 255, 0,
        255); // pure green        this->buttonTexture =
              // CreateSolidTexture(600, 80, 0, 255, 255, 255);
    // dalam InitGame(), selepas CreateSolidTexture
    ...
    if (!this->buttonTexture)
      throw engine::error("buttonTexture NULL – CreateSolidTexture failed: %s",
                          SDL_GetError());
    this->lua.open_libraries(sol::lib::base, sol::lib::string,
                             sol::lib::coroutine);

    this->lua.set_function("bg",
    ...
    this->lua.set_function("hide",
                           [this](const std::string &c) { HideCharacter(c); });
    this->lua.set_function("say", [this](sol::variadic_args va) {
    ...
    this->lua.set_function("narrate",
                           [this](const std::string &t) { Narrate(t); });
    this->lua.set_function("wait", [this]() {});
  ...
  void EnterMenu() {
    ...
    std::cout << "[MENU] Loaded background and " << buttonTexts.size()
              << " buttons!" << std::endl;
  }
  ...
  void Draw() {
    ...
    for (const auto &[id, tex] : characters) {
      ...
      SDL_FRect dst{xPos, screenHeight - th * scale - screenHeight * 0.1f,
                    tw * scale, th * scale};
    ...
    if (inMenu) {
      ...
      for (size_t i = 0; i < buttonTexts.size(); ++i) {
        ...
        if (textTex) {
          ...
          SDL_FRect tr{float(screenWidth) / 2 - tw / 2, y + 40 - th / 2, tw,
                       th};
    ...
    if (!currentText.empty()) {
      if (textBox) {
        SDL_FRect tb{0, screenHeight * 0.75f, (float)screenWidth,
                     screenHeight * 0.25f};
  ...
  SDL_Texture *LoadTexture(const std::string &path) {
    SDL_Texture *tex =
        IMG_LoadTexture(this->renderer, ("assets/bg/" + path).c_str());
    if (!tex)
  ...
  SDL_Texture *RenderText(const std::string &text, SDL_Color color) {
    if (text.empty() || !this->font)
      return nullptr;
    SDL_Surface *surf =
        TTF_RenderText_Blended(this->font, text.c_str(), text.size(), color);
    if (!surf)
      return nullptr;
  ...
  void LoadScript(const std::string &filename) {
    ...
    if (!chunk.valid()) {
      sol::error err = chunk;
      std::cerr << "[ERROR] Lua load error in " << filename << ": "
                << err.what() << "\n";
      return;
    }
    script = sol::coroutine(chunk);
    scriptFinished = false;
    std::cout << "[DEBUG] Script loaded successfully, starting execution..."
              << std::endl;
    std::cout << "[DEBUG] Initial run complete. Status: "
              << (int)script.status();
  }
  ...
  int HitTestButton(int mx, int my) {
    float y = screenHeight * 0.4f;
    const float h = 80;
...
CerekaEngine::~CerekaEngine() { delete pImplementation; }

bool CerekaEngine::InitGame(const char *title, int w, int h, bool fullscreen) {
  return pImplementation->InitGame(title, w, h, fullscreen);
}

void CerekaEngine::ShutDown() { pImplementation->ShutDown(); }

bool CerekaEngine::PollEvent(CerekaEvent &e) {
  return pImplementation->PollEvent(e);
}

void CerekaEngine::Present() { pImplementation->Present(); }

int CerekaEngine::Width() const { return pImplementation->screenWidth; }

int CerekaEngine::Height() const { return pImplementation->screenHeight; }

void CerekaEngine::LoadScript(const std::string &filename) {
  pImplementation->LoadScript(filename);
}

void CerekaEngine::Reset() { pImplementation->Reset(); }

void CerekaEngine::HandleEvent(const CerekaEvent &e) {
  pImplementation->HandleEvent(e);
}

void CerekaEngine::Update(float dt) { pImplementation->Update(dt); }

void CerekaEngine::Draw() { pImplementation->Draw(); }
...
void CerekaEngine::LoadCompiledScript(
    const std::vector<scenario::Instruction> &compiled) {
  pImplementation->LoadCompiledScript(compiled);
}

void CerekaEngine::AdvanceScriptOnce() { pImplementation->AdvanceScriptOnce(); }
bool CerekaEngine::InMenu() const { return pImplementation->inMenu; }
...
size_t CerekaEngine::ProgramCounter() const { return pImplementation->pc; }

void CerekaEngine::TickScript() { return pImplementation->TickScript(); }
